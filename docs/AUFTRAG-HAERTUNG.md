...
> | **H2** Konfiguration über die Umgebung | **fertig, bis auf eine Entscheidung.** `.env`/`.env.example`/`make env` stehen, `Umgebung.Laden()` ist erste Zeile in allen fünfzehn `Program.cs` und ein Test hält das fest. Die Geheimnisfrage ist **gemessen** (12.09.2026, [`MESSUNG-GEHEIMNISFRAGE.md`](MESSUNG-GEHEIMNISFRAGE.md)): keines der drei Module berührt sie — `AddSecretManagement` läuft längst und tut nichts, `AddEncryption` würde den Klartext ablegen, `AddResourceAuthorization` trägt nichts. Was bleibt, ist keine Messung, sondern eine Entscheidung: eigener Geheimnisspeicher als Dienst, oder Umgebung als Mechanismus. |
