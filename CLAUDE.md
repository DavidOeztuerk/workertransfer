...
WorkerTransfer is a consent-first talent-mobility platform (applications, direct recruiting, employment transfers, AI-assisted career workflows). It is **.NET 10 on [Girder](https://github.com/DavidOeztuerk) 4.1.0** — a shared foundation library of this author's, pulled from GitHub Packages — plus a React app.
...
**`Bremse.cs` stays, and the reason changed with 4.1.0.** It used to be the shape of the rejection — `application/json` with a `traceId`, no problem document and no correlation id. That landed: Girder's 429 is now `application/problem+json` naming the correlation id, and the seven foreign per-path defaults are gone. What remains is a difference in *kind*, and it does not move: **Girder's middleware is a global brake with per-path refinement** — every path without an entry of its own gets the default limit (`endpointSpecificLimits ?? defaultLimits`). The whole UI travels through this gateway, so a default limit over everything would count every asset fetch. Ours brakes **five named paths** and touches nothing else. Setting the default to an unreachable number would put a limit in configuration that is not one.
...
| `Korrelation.cs` (Gateway) | **ja — die Frage ist beantwortet** | Girders Zwischenschicht schrieb die Kennung bis 4.0.2 nur in Antwortkopf, Gepäck und `Items`; ein Reverse Proxy reicht aber nur **Anfrage**köpfe weiter, also erfand jeder Dienst dahinter eine eigene. Gemeldet, seit 4.1.0 behoben. Sie bleibt hier trotzdem: dieses Gateway ruft kein `AddGirder` und hat weder Datenbank noch Endpunkte, die ein Token prüfen. Für zwanzig Zeilen das ganze Modulsystem einzuhängen ist ein schlechter Tausch. |
