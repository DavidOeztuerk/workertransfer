...
Die Tür zum Transfermarkt. Ohne sie entsteht `market.visibility:tenant:<id>`
nirgends, und ein Vorgang (5.2) könnte nie beginnen.
...
jetzt besteht, beantwortet ausschließlich der Consent-Ledger, frisch bei jedem
Zugriff (ADR-0013). Deshalb gibt es hier weder ein `is_active` noch ein
`revoked_at`. Nach einem Widerruf bleibt die Anfrage `GRANTED` und der Zugriff
läuft trotzdem ins Leere — kein Widerspruch, sondern die Trennung zwischen dem,
was geschehen ist, und dem, was gilt.
"""
