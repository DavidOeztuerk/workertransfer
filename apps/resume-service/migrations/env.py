"""Async Alembic env.py (ADR-0010) for the resume-service database.
...
# Make `resume_service` importable when alembic runs from apps/resume-service.
...
def _database_url() -> str:
    return os.environ.get(
        "WORKER_DATABASE_URL",
        os.environ.get(
            "DATABASE_URL", "postgresql+asyncpg://worker:worker@127.0.0.1:5432/resume_service"
        ),
    )
