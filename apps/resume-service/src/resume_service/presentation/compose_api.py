"""Composition-Root der Resume-Service-App (ADR-0003).

Explizit statt fluent: ein Leser sieht in dieser einen Datei, welche
Querschnittsthemen aktiv sind. Der Service stellt keine Tokens aus, er
verifiziert nur die von identity-service (ADR-0015) — deshalb dieselbe geteilte
JwtAuthMiddleware wie consent- und profile-service.
"""
...
from resume_service.infrastructure.compose import compose_infrastructure, outbox_runner
from resume_service.presentation.http.erasure_router import build_erasure_router
from resume_service.presentation.http.router import build_router
...
def build_app(settings: ResumeServiceSettings) -> FastAPI:
