...
class NotificationPreferenceModel(Base):
    ...
    Kein Fremdschlüssel auf `users`: die Zeile entsteht beim ersten Bedarf, und
    ein fehlender Datensatz bedeutet „alles an" (siehe domain/notification.py).
    """

    __tablename__ = "notification_preferences"

    user_id: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), primary_key=True)
    resume_request: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    market_request: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    application_update: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    transfer_update: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
