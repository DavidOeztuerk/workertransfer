"""Async Alembic env.py (ADR-0010) for the github-service database.
...
# Make `github_service` importable when alembic runs from apps/github-service.
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

# Import models so their tables register on Base.metadata (autogenerate target).
from github_service.infrastructure.database import models as _models  # registers tables
...
def _database_url() -> str:
    return os.environ.get(
        "WORKER_DATABASE_URL",
        os.environ.get(
            "DATABASE_URL", "postgresql+asyncpg://worker:worker@127.0.0.1:5432/github_service"
        ),
    )
