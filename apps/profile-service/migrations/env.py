"""Async Alembic env.py (ADR-0010) for the profile-service database.
...
# Make `profile_service` importable when alembic runs from apps/profile-service.
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

# Import models so their tables register on Base.metadata (autogenerate target).
from profile_service.infrastructure.database import models as _models  # registers tables
...
def _database_url() -> str:
    return os.environ.get(
        "WORKER_DATABASE_URL",
        os.environ.get(
            "DATABASE_URL", "postgresql+asyncpg://worker:worker@127.0.0.1:5432/profile_service"
        ),
    )
